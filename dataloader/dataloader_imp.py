...
class CustomTrainerForgetting(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        def get_batch_loss_mask(logits,labels,mask_):
            ...
            loss = (shift_key.view(-1).cuda() *f_ce).sum(dim=-1)
            return loss
        ...
        if self.loss_type=='ga_imp':
            forget_inputs, retain_inputs = inputs
            #pdb.set_trace()
            input_ids, labels, attention_mask, soft_mask = forget_inputs
            outputs = model(input_ids,labels=labels, attention_mask=attention_mask)
            shift_logits = outputs.logits[..., :-1, :].contiguous()
            labels = labels.to(outputs.logits.device)
            shift_labels= labels[..., 1:].contiguous()
            shift_key = soft_mask[..., 1:].contiguous()
            f_ce=nn.CrossEntropyLoss(ignore_index=-100, reduction = 'none')(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            loss = -1.*(shift_key.view(-1).cuda() *f_ce).sum(dim=-1)

        if self.loss_type=='gd_imp':
            forget_inputs, retain_inputs = inputs
            #pdb.set_trace()
            input_ids, labels, attention_mask, soft_mask = forget_inputs
            outputs = model(input_ids,labels=labels, attention_mask=attention_mask)
            ...
            shift_key = soft_mask[..., 1:].contiguous()
            f_ce=nn.CrossEntropyLoss(ignore_index=-100, reduction = 'none')(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            forget_loss = -1.*(shift_key.view(-1).cuda() *f_ce).mean(dim=-1)

            retain_input_ids, retain_labels, retain_attention_mask, retain_mask = retain_inputs
            retain_outputs = model(retain_input_ids,labels=retain_labels, attention_mask=retain_attention_mask)
            retain_shift_logits = retain_outputs.logits[..., :-1, :].contiguous()
            retain_shift_labels = retain_labels[..., 1:].contiguous()
            r_ce = nn.CrossEntropyLoss(ignore_index=-100, reduction = 'none')(retain_shift_logits.view(-1, retain_shift_logits.size(-1)), retain_shift_labels.view(-1))
            retain_loss = 1. * (r_ce).mean(dim=-1)
            with open(f'{self.log_dir}/influence_record.txt','a') as file:
                file.write(f"{shift_key.view(-1).tolist()}\n")
                file.write(f"{f_ce.tolist()}\n")

            loss = forget_loss + retain_loss

        if self.loss_type == 'npo_imp': 
            ...
            input_ids, labels, attention_mask,soft_mask = forget_inputs
            outputs = model(input_ids,labels=labels, attention_mask=attention_mask)
            forget_loss_current = get_batch_loss_mask(outputs.logits, labels,soft_mask) 
            with torch.no_grad():
                forget_outputs_oracle = self.oracle_model(input_ids,labels=labels, attention_mask=attention_mask)
                forget_logits_oracle = forget_outputs_oracle.logits
                forget_loss_oracle = get_batch_loss_mask(forget_logits_oracle, labels, soft_mask)
            neg_log_ratios = forget_loss_current - forget_loss_oracle

            loss = -F.logsigmoid(self.beta * neg_log_ratios).mean() * 2 / self.beta 
        
        if self.loss_type == 'npo_gd_imp': 
            if self.beta_param != 'None':
                self.beta = self.beta_param
            else: self.beta=.5
            ...
            retain_input_ids, retain_labels, retain_attention_mask,soft_mask_retain = retain_inputs
            retain_outputs = model(retain_input_ids,labels=retain_labels, attention_mask=retain_attention_mask)
            retain_loss = retain_outputs.loss
            loss = self.coeff * forget_loss + retain_loss
            
        if self.loss_type == 'npo_kl_imp': 
