...
class CustomTrainerForgetting(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.loss_type == 'satimp':
            ...
            weight_part = (- lm_loss).exp().detach() ** self.hyper_param
            weight_ce = (1-(- lm_loss).exp().detach()) ** self.beta
            loss = -((weight_ce * weight_part) * lm_loss)[shift_labels.view(-1)!=-100].mean()
        
        if self.loss_type == 'satimp_gd':
            if self.hyper_param != '':
                self.beta =self.beta_param
            else:
                self.beta = 5
            ...
            loss = forget_loss + self.coeff * retain_loss

        if self.loss_type == 'simsat_ga':
            if self.hyper_param != 'None':
                self.beta = self.beta_param
            else: 
                self.beta = 5
            ...
            shift_labels = labels[..., 1:].contiguous()
            lm_loss = CrossEntropyLoss(ignore_index= -100, reduction = 'none')(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            weight_ce = (- lm_loss).exp().detach()
            #weight_ce = ((weight_part**self.beta) /((weight_part**self.beta) +self.hyper_param))
            loss = -(weight_ce * lm_loss)[shift_labels.view(-1)!=-100].mean()
            with open(f'{self.log_dir}/weight.txt','a') as file:
                file.write(f"{weight_ce[shift_labels.view(-1) != -100].tolist()}\n")
                file.write(f"{lm_loss.detach()[shift_labels.view(-1) != -100].tolist()}\n")

        if self.loss_type == 'simimp_ga':
            if self.hyper_param != 'None':
        ...
        if self.loss_type == 'simsat_gd':
            ...
            shift_labels = labels[..., 1:].contiguous()
            lm_loss = CrossEntropyLoss(ignore_index= -100, reduction = 'none')(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            weight_part = (- lm_loss).exp().detach()
            #weight_ce = (weight_part/(weight_part+self.hyper_param)) ** self.beta
