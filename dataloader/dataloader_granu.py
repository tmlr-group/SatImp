...
from data_module_imp import get_batch_loss
...
class CustomTrainerForgetting(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        #self.logging_step=kwargs.pop('log_steps')
        self.count_step=0
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        def group_and_average(a,b, group_size=10):
            num = int(len(a)/499)
            a_ = a.reshape(num,499)
            b_ = b.reshape(num,499)
            mask = (b_ != 0.0000).float()
            valid_a = a_ * mask

            sorted_a, sorted_indices = valid_a.sort(dim=-1, descending=True)
            num_valid_tokens = mask.sum(dim=-1).long()  
            num_groups = (num_valid_tokens + group_size - 1) // group_size  
    
            new_valid_a = torch.zeros_like(valid_a)

            for i in range(valid_a.shape[0]):
                valid_indices = sorted_indices[i]
                valid_values = sorted_a[i]
                valid_mask = mask[i]
        
                group_start_idx = 0
                for group_idx in range(num_groups[i]):
                    group_end_idx = min(group_start_idx + group_size, num_valid_tokens[i])
                    group_values = valid_values[group_start_idx:group_end_idx]
                    group_avg = group_values.mean().item()
            
                    new_valid_a[i, valid_indices[group_start_idx:group_end_idx]] = group_avg
                    group_start_idx = group_end_idx
    
            new_a=new_valid_a
            new_a_ = new_a.reshape(len(a))
            return new_a_


        if self.loss_type == 'instance_simsat_ga':
            ...
            lm_loss = get_batch_loss(outputs.logits,labels)
            weights = (- lm_loss).exp().detach() ** self.beta
            loss = -(weights * lm_loss).mean()
        
        if self.loss_type == 'instance_simimp_ga':
            if self.hyper_param != 'None':
                self.beta = self.hyper_param
            else: 
                self.beta = 5
            ...
            labels = labels.to(outputs.logits.device)
            lm_loss = get_batch_loss(outputs.logits,labels)
            weights = (1-(- lm_loss).exp().detach()) ** self.beta
            loss = -(weights * lm_loss).mean()

        if self.loss_type == 'instance_simsat_gd':
            if self.hyper_param != 'None':
                self.beta = self.hyper_param
            else: 
                self.beta = 5
            ...
            lm_loss = get_batch_loss(outputs.logits,labels)
            weights = (- lm_loss).exp().detach() ** self.beta
            forget_loss = -(weights * lm_loss).mean()
        ...
        if self.loss_type == 'instance_simimp_gd':
            if self.hyper_param != 'None':
                self.beta = self.hyper_param
            else: 
                self.beta = 5
        ...
        if self.loss_type == 'group_simsat_ga':
            ...
            weights = (- lm_loss).exp().detach() ** self.beta
            instanceweight=group_and_average(weights,lm_loss.detach(),self.hyper_param)
            loss = -(instanceweight * lm_loss)[shift_labels.view(-1)!=-100].mean()
        
        if self.loss_type == 'group_simimp_ga':
            if self.beta_param != 'None':
                self.beta = self.beta_param
            else: 
                self.beta = 5
        ...
        if self.loss_type == 'group_simsat_gd':
            if self.beta_param != 'None':
                self.beta = self.beta_param
            else: 
                self.beta = 5
            ...
            weights = (- lm_loss).exp().detach() ** self.beta
            instanceweight=group_and_average(weights,lm_loss.detach(),self.hyper_param)
            forget_loss = -(instanceweight * lm_loss)[shift_labels.view(-1)!=-100].mean()

            retain_input_ids, retain_labels, retain_attention_mask = retain_inputs
            retain_outputs = model(retain_input_ids,labels=retain_labels, attention_mask=retain_attention_mask)
            retain_loss = retain_outputs.loss

            with open(f'{self.log_dir}/weight.txt','a') as file:
                file.write(f"{instanceweight[shift_labels.view(-1) != -100].tolist()}\n")
                file.write(f"{lm_loss.detach()[shift_labels.view(-1) != -100].tolist()}\n")
            loss = forget_loss + retain_loss
        
        if self.loss_type == 'group_simimp_gd':
            if self.beta_param != 'None':
                self.beta = self.beta_param
            else: 
                self.beta = 5
        ...
        if self.loss_type == 'batch_simsat_ga':
            if self.hyper_param != 'None':
                self.beta = self.hyper_param
            else: 
                self.beta = 5
            ...
            lm_loss = get_batch_loss(outputs.logits,labels)
            weights = (- lm_loss).exp().detach() ** self.beta
            batchweight=weights.mean()
            loss = -(batchweight.item() * lm_loss).mean()
        
        if self.loss_type == 'batch_simimp_ga':
            if self.hyper_param != 'None':
                self.beta = self.hyper_param
            else: 
                self.beta = 5
        ...
        if self.loss_type == 'batch_simsat_gd':
            if self.hyper_param != 'None':
                self.beta = self.hyper_param
            else: 
                self.beta = 5
            ...
            weights = (- lm_loss).exp().detach() ** self.beta
            batchweight=weights.mean()
            forget_loss = -(batchweight.item() * lm_loss).mean()
        ...
        if self.loss_type == 'batch_simimp_gd':
            if self.hyper_param != 'None':
                self.beta = self.hyper_param
            else: 
                self.beta = 5
